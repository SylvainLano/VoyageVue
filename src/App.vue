  ...
  <div class="absolute origin-top-left z-20 top-0" :style="{ width: `${widthMenuSize}px`, transform: `scale(${scaleFactor})` }">
    ...
    <div class="relative bg-slate-50 p-4" :style="{ height: `${heightMenuSize}px`}" v-if="showMenu">
      <div class="font-mono text-4xl text-slate-900 text-left flex justify-between">
        ...
        <div class="text-slate-900 cursor-pointer" v-if="!isStarred(currentDestination.id) && !isBanned(currentDestination.id)" @click="toggleReservationMenu, starToggle" title="Add this destination to your favorites!">
          <font-awesome-icon :icon="['far', 'star']" :bounce="isBeating" :transform="'shrink-'+starShrink" />
        </div>
        <div class="text-emerald-400 cursor-pointer" v-else-if="!isBanned(currentDestination.id)" @click="toggleReservationMenu, starToggle" title="Remove this destination from your favorites.">
